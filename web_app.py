      ...
      <div class="nox-voice-widget" id="noxVoiceWidget" data-csrf="{token}" aria-live="polite">
        ...
        <section class="nox-voice-panel" id="noxVoicePanel" role="dialog" aria-label="Assistant vocal NOX-IA">
          ...
          <div class="nox-voice-chat" id="noxVoiceChat">
            <div class="nox-voice-msg ai">Salut. Je suis NOX. Tu peux me parler, m’écrire, ou activer le bouton « NOX » pour m’appeler simplement en disant mon nom.</div>
          </div>
...
let isOpen=false,isBusy=false,isListening=false,recognition=null,recognitionFinal='',recognitionShouldSend=false,speakEnabled=true,wakeEnabled=false,wakeRecognition=null,wakeRunning=false,wakeBlocked=false,wakeTimer=null,wakeCommandArmed=false;
...
function openPanel(){isOpen=true;panel.classList.add('open');repositionPanel();setTimeout(()=>input.focus(),60)}function closePanel(){isOpen=false;panel.classList.remove('open');stopListening();if(wakeEnabled)scheduleWake(350)}function clamp(n,a,b){return Math.max(a,Math.min(b,n))}
...
function startWake(){if(!wakeEnabled||wakeRunning||wakeBlocked||isBusy||isListening||document.hidden)return;const Ctor=recognitionCtor();if(!Ctor){wakeEnabled=false;wakeBtn.classList.remove('active');orb.classList.remove('wake-active');try{localStorage.setItem(WAKE_KEY,'0')}catch(e){}setStatus('Le mot-clé NOX n’est pas pris en charge par ce navigateur.');return}wakeRecognition=new Ctor();wakeRecognition.lang='fr-FR';wakeRecognition.continuous=true;wakeRecognition.interimResults=true;wakeRecognition.maxAlternatives=1;wakeRecognition.onstart=()=>{wakeRunning=true;wakeBlocked=false;setStatus('Dis « NOX » quand tu as besoin de moi.',true)};wakeRecognition.onresult=e=>{for(let i=e.resultIndex;i<e.results.length;i++){const res=e.results[i],heard=res[0].transcript||'',hit=wakePhrase(heard);if(hit&&hit.found){/* Sur un résultat intermédiaire « NOX », on attend la suite pour ne pas couper « NOX caméra Hikvision ». */if(!hit.command&&!res.isFinal)continue;wakeCommandArmed=true;wakeBlocked=true;stopWake(true);openPanel();setOrbState('listening');setStatus(hit.command?'NOX détecté · j’exécute…':'NOX détecté · parle maintenant…',true);addMessage('system','Mot-clé « NOX » détecté.');if(hit.command){input.value=hit.command;setTimeout(()=>ask(hit.command),90)}else setTimeout(()=>{wakeBlocked=false;startListening()},100);break}}};wakeRecognition.onerror=e=>{wakeRunning=false;const c=e&&e.error?e.error:'';if(c==='not-allowed'||c==='service-not-allowed'){wakeEnabled=false;wakeBlocked=false;wakeBtn.classList.remove('active');orb.classList.remove('wake-active');try{localStorage.setItem(WAKE_KEY,'0')}catch(err){}setStatus('Autorise le micro pour activer « NOX ».')}};wakeRecognition.onend=()=>{wakeRunning=false;if(wakeEnabled&&!wakeBlocked)scheduleWake(550)};try{wakeRecognition.start()}catch(e){wakeRunning=false;scheduleWake(1100)}}
...
def _voice_email_command(text_value):
    ...
    if not re.search(r'\b(?:mail|e-?mail|email)\b',q,re.I):return None
    m=re.search(r'\b(?:mail|e-?mail|email)\b\s+(?:à|a|pour)\s+(.+)$',q,re.I)
    if not m:return None
    rest=m.group(1).strip()
    body_match=re.search(r'\s+(?:et\s+)?(?:écris|ecris|écrit|ecrit|mets|met|dis|dit|avec\s+comme\s+message|message)\s+(.+)$',rest,re.I)
    if body_match:
        body=body_match.group(1).strip();prefix=rest[:body_match.start()].strip(' ,:-')
    elif ':' in rest:
        prefix,body=rest.split(':',1);prefix=prefix.strip();body=body.strip()
    else:return None
    subject=''
    sm=re.search(r'^(.*?)\s+(?:avec\s+)?(?:pour\s+)?(?:sujet|objet)\s+(.+)$',prefix,re.I)
    if sm:
        target=sm.group(1).strip(' ,:-');subject=sm.group(2).strip(' ,:-')
...
def assistant_voice_command(request:Request,question:str=Form(...),page_path:str=Form(''),wake_invoked:str=Form('0'),csrf_token_value:str=Form(...,alias='csrf_token'),db:Session=Depends(get_db)):
